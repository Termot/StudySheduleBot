...
class VkBot:
    def __init__(self, user_id):
        self._CONN = sqlite3.connect('user_config.db')  # подключаемся к базе данных
        self._CUR = self._CONN.cursor()  # наводим курсор в базу данных

        self._VK = vk_api.VkApi(token=tokens('vk_tc_test'))  # Авторизуемся как сообщество
    ...
    def check_valid_data(self, message_list):
        if self._CUR.execute('SELECT userid FROM users_config WHERE факультет is NULL'):
            pass
        else:
    ...
    def user_data_writer(self, data):                        # записываем данные пользователя в базу данных user_config
        ...
        if data_list[0] == 'ФАКУЛЬТЕТ':
            ...
            WHERE userid = {self._USER_ID};''')
            self._CONN.commit()
            ud = self._VK.method('users.get', {'user_id': self._USER_ID})[0]
...
Группа:        {rows[6]}''')
    ...
    def user_config(self):
        ...
        if self._CUR.fetchone() is None:
            self._CUR.execute('''INSERT INTO users_config(userid, фамилия, имя)
...
def start_vk_bot():
    token = tokens('vk_tc_test')  # API-ключ с доступом к сообщениям сообщества
